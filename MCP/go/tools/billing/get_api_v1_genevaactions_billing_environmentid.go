...
func Get_api_v1_genevaactions_billing_environmentidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/GenevaActions/Billing/%s%s", cfg.BaseURL, environmentId, queryString)
...
func CreateGet_api_v1_genevaactions_billing_environmentidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_GenevaActions_Billing_environmentId",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_api_v1_genevaactions_billing_environmentidHandler(cfg),
	}
}
