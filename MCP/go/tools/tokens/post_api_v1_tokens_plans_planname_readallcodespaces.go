...
func Post_api_v1_tokens_plans_planname_readallcodespacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/Tokens/plans/%s/readAllCodespaces%s", cfg.BaseURL, planName, queryString)
...
func CreatePost_api_v1_tokens_plans_planname_readallcodespacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_Tokens_plans_planName_readAllCodespaces",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_tokens_plans_planname_readallcodespacesHandler(cfg),
	}
}
