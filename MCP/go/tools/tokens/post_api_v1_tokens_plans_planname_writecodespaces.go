...
func Post_api_v1_tokens_plans_planname_writecodespacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/Tokens/plans/%s/writeCodespaces%s", cfg.BaseURL, planName, queryString)
...
func CreatePost_api_v1_tokens_plans_planname_writecodespacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_Tokens_plans_planName_writeCodespaces",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_tokens_plans_planname_writecodespacesHandler(cfg),
	}
}
