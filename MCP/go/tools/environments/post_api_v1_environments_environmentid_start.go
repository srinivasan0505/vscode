...
func Post_api_v1_environments_environmentid_startHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/Environments/%s/start%s", cfg.BaseURL, environmentId, queryString)
...
func CreatePost_api_v1_environments_environmentid_startTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_Environments_environmentId_start",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_environments_environmentid_startHandler(cfg),
	}
}
