...
func Post_api_v1_tenant_tenantid_pool_poolname_vm_vmname_stopHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/tenant/%s/pool/%s/Vm/%s/stop", cfg.BaseURL, tenantId, poolName, vmName)
...
func CreatePost_api_v1_tenant_tenantid_pool_poolname_vm_vmname_stopTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_tenant_tenantId_pool_poolName_Vm_vmName_stop",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_tenant_tenantid_pool_poolname_vm_vmname_stopHandler(cfg),
	}
}
