...
func Get_api_v1_tenant_tenantid_pool_poolname_vmHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/tenant/%s/pool/%s/Vm", cfg.BaseURL, tenantId, poolName)
...
func CreateGet_api_v1_tenant_tenantid_pool_poolname_vmTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_tenant_tenantId_pool_poolName_Vm",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_api_v1_tenant_tenantid_pool_poolname_vmHandler(cfg),
	}
}
