...
func Get_api_v1_locations_locationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/Locations/%s%s", cfg.BaseURL, location, queryString)
...
func CreateGet_api_v1_locations_locationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_Locations_location",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_api_v1_locations_locationHandler(cfg),
	}
}
